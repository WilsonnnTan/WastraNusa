...
export class HTMLStateMachineParser {
  ...
  private static readonly WIKIPEDIA_ARTIFACTS = [
    /\[\s*sunting\s*\|\s*sunting\s+sumber\s*\]/gi, // [ sunting | sunting sumber ]
    /\[\s*edit\s*\|\s*edit\s+source\s*\]/gi, // [ edit | edit source ]
    /\{\{[^}]*\}\}/g, // {{templates}}
    /\[\d+\]/g, // [1], [2], etc
    /\^/g, // ^ carets
    ...
    /\.mw-parser-output[^}]*\}/g, // .mw-parser-output css blocks
    /@media[^}]*\}[^}]*\}/gi, // @media rules
    /body:not\([^)]*\)[^}]*\}/g, // body:not() rules
    /\.\w+\s*\{[^}]*\}/g, // .classname { style }
    /url\(".*?"\)/gi, // url("...") backgrounds
    /rgba?\([^)]*\)/g, // rgb/rgba colors
    /\d+\.\d+em|\d+px|%|em|rem/g, // CSS measurements (be careful - may remove legitimate measurements)
  ];
